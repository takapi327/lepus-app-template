...
class CategoryService @Inject()(
  ...
  def getAll: IO[Seq[JsValueCategory]] =
    ...
    yield categories.map(JsValueCategory.build)).transact(slave)

  def add(category: JsValuePostCategory): IO[Long] =
    categoryRepository.add(Category(None, category.name, category.slug, category.color)).transact(master)
  ...
  def delete(id: Long): IO[Int] = categoryRepository.delete(id).transact(master)
